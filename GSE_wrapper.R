...
run_camera <- function(expr_log2, sample.cat=c(1,1,2,2), genome=c("hg19", "mm10"), geneID=c("geneSymbol"),
  ...
  if(gs_enrich_plot) {
    ...
    p <- ggplot(df, aes(x=type, y=expr)) + geom_boxplot(aes(color=type), outlier.size=1) + geom_point() + 
                 facet_wrap(~ ID, scales="free",nrow=plot.nrow,ncol=plot.ncol, labeller=label_wrap_gen(width=35)) + 
                 theme_Publication() + theme(strip.text = element_text(face="plain", size=rel(0.3)))
    return(p)
    ## genes       
